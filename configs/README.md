...
（Raspberry Pi Mouse では同じインタフェースが本当にステップクロックで、名前はそこから
引き継いだもの）。**基板から車輪径は見えない**ので、下の改造は基板の中のループには
影響せず、換算の側だけの問題として閉じます。
...
違います**（比のほうは 142 秒の計測なので 0.5% 以下の精度で出ています）。エンコーダが
数えるものと基板が指令として受け取るものが別、ということです。上流はこれを 1 つの数で兼ねて
...
上の比 1.960 のほうは 1073/447 = 2.40 へ動いていますが、**別物という結論は同じ**です。

`raspimouse` ノード（`driver:=raspimouse`）はここが直せません。`cmd_vel` → 指令
周波数の換算に **400 を直書き**していて、パラメータはオドメトリ側にしか効かないため
です。結果として、この機体を公式実装で走らせると**指令の 89% の速度**で走ります
（`400 / 447`）。`use_pulse_counters: false` なので `odom` は指令値の積分、つまり
**指令どおりの値を報告し続けます**。ログにも `odom` にも何も出ません。

寸法を実測値へ直した 2026-08-03 の修正で、この誤差の向きが変わりました。上流の
車輪径 0.1524 と直書きの 400 は互いに逆を向いていて（`0.200/0.1524 × 400/447 =
1.17`）、径が古いままだと 17% 速すぎます。径だけを正すと 89% になります。**径を
...
変位を比べる）は、荷重が乗った状態でのすべりと基板のループの飽和を見るために別途必要です。
...
* **`pulses_per_revolution`（1073.0）と `steps_per_revolution`（447.0）が別のキー**
  です。前者はエンコーダのパルス数で `odom` 専用、後者は基板への指令パルス数で
  `cmd_vel` → 周波数専用。上の実測のとおり 2 倍以上違うので、上流のように 1 つで兼ねると、機体は
...
重みは尤度場（壁から `laser_likelihood_max_dist` で 0 に落ちる線形）を**全ビーム
（0.5 度刻みの 720 本）ぶん足した和**なので、数 cm ずれた粒子は 1 回の更新で大きく
負けます。結果として出力は実質スキャンマッチャそのものになり、オドメトリは探索範囲を
与えるだけの役になっていました（当時の `laser_likelihood_max_dist` は既定の 0.2 m。
**2026-08-09 に 1.0 m へ広げた**ので、いまは 1 本あたりの落ち方が 1/5 で緩く、
粒子間に差が付きにくい側です。下の別項）。

2026-08-07 に 0.05 / 0.02 / 0.05（1 m 直進で 0.05 m・1.15 度、90 度旋回で 3.6 度）まで
下げ、**2026-08-09 に 0.10 / 0.05 / 0.10（1 m 直進で 0.10 m・2.9 度、90 度旋回で
7.2 度）へ上げ直しました**。既定の約半分で、実測の実力よりはまだ数倍広い側です。
**この上げ直しの根拠は記録されておらず、未検証です。** 潰さないこと。
...
なります。加えて、事前分布を約 2 倍狭めた今は**二度食いの効きも小さくなって
います**（狭い事前分布に同じ証拠を 2 回掛けても、広い事前分布のときほど動かない。
2026-08-09 に上げ直す前は 4〜6 倍狭めていたので、そのぶん戻っています）。
...
ですが、**この 1 つの値が重み付けと貫通判定の両方を決めます**。効きが大きいのは
後者のほうです。

尤度場は占有セル（`v > 50`）ごとに `LikelihoodFieldMap::setLikelihood` が正方形に
書き込みます。`cell_num = ceil(range / resolution)` として、重みは
`255 × (1 − i / cell_num)`（`i` は**チェビシャフ距離**＝縦横の大きいほう）で、
セルどうしは `max` で合成されます。0.05 m/cell なので `cell_num` は
**0.2 m で 4、1.0 m で 20**。円ではなく正方形なので、対角方向へは `range × √2` まで
届きます。

**重み付け側**（`Particle::likelihood`）: 1 本のビームの寄与は 255 から 0 へ落ちる
までの距離がそのまま `cell_num` 段になるので、幅を 5 倍にすると数 cm のずれに対する
感度が **1/5** になります。粒子どうしの差が付きにくくなる方向で、2026-08-09 に
運動モデルを広げた（上の節）のと合わせて、オドメトリ寄りへ 2 段振ったことになります。

**貫通判定側**（`Particle::isPenetrating`）: レイを刻んで進み、`likelihood == 255`
（＝占有セルそのもの。ここは幅を変えても動きません）を踏んだ後に
**`likelihood == 0` のセルへ抜けたら「貫通」**と数えます。この `== 0` が
「どの占有セルからもチェビシャフ距離で `laser_likelihood_max_dist` 以上」の意味なので、
**幅がそのまま「壁の先がどれだけ離れていれば貫通と見なすか」になります**。

これが 19F に効く理屈です。地図の壁と実物の壁が 0.3 m ずれていると、レイは地図の壁を
踏んでから 0.3 m 先の実測点で終わりますが、そこは地図の壁から 0.3 m しか離れていない
ので、1.0 m ではもう「空き」になりません＝**貫通と数えられません**。一方、本当に
自己位置を見失って壁を突き抜け広い空間へ出たレイは 1 m 以上先まで届くので、**そちらは
今までどおり捕まります**。つまり「サブメートルの地図誤差には目をつぶり、大きな
食い違いだけ拾う」判定へ変わります。`alpha`（非貫通率）が上がるぶん
`overrides/map_19f.yaml` の `alpha_threshold: 0.2` による膨張リセットは起きにくく
なります。

**ただしこれは連続な鈍り方ではなく、通路幅で切り替わる崖です。** 零セルの条件が
「どの占有セルからも 1.0 m 以上」なので、両側に壁のある**幅 2.0 m 未満の空間には
零セルが 1 つも存在しません**。そこでは `isPenetrating` が構造的に `true` を返せず、
貫通判定は鈍るのではなく**止まり**、`alpha` は 1.000 に張り付きます。19F の実際の
通路幅次第で「大きな食い違いも拾わない」側に落ちるということで、**幅の上限を決めて
いるのは地図ではなく通路のほう**です。狭ければ 0.5 m（幅 1.0 m 相当）へ落とすこと。

**裏を返せば、幅を広げることは貫通検知を鈍らせることです。** 19F の根因（地図と実環境の
不整合）が直るわけではなく、`alpha_threshold` を下げたのと同じ**対症療法をもう 1 段
重ねた**形です。地図を取り直したら、`alpha_threshold` の 2 つと合わせてここも既定へ
戻すこと。

参考値は emcl2 のノード既定 0.2 だけが極端に狭い側です（`open-rdc/orne-box` 0.8、
`uhobeike/raspicat_navigation` 2.0、Nav2 AMCL の既定と当リポジトリの `amcl.yaml` は
どちらも 2.0）。

**未検証**。確かめ方はログの `ALPHA: <値> / <閾値>` と `RESET` の頻度で、**3 通りを
区別すること**です。

* alpha が 0.0〜0.4 の張り付きから **0.5 前後まで上がり、`RESET` が減った** … 狙いどおり。
* alpha が **1.000 に張り付き `RESET` が一度も出ない** … 上の崖に落ちて**判定が死んで
  います**。効いたのではないので、0.5 あたりへ落として測り直すこと。
* alpha が **上がらない** … 根因が地図誤差ではありません（TF の遅延・オドメトリ・
  仰角フィルタ由来の偽の壁）。

**この値は断片に置いてあるので両方の地図に効きます。** 地図ごとに変えたくなったときは
`overrides/*.yaml` の `emcl2` の節へ移すこと（そのときは 2 つの override にキーを
そろえて並べる）。起動時のコストは占有セル 1 つあたり `(2·cell_num+1)²` の書き込みで
81 → 1681 と 20.8 倍になりますが、`setLikelihood` が走るのは `v > 50` のセルだけで、
未観測（`map_server` は -1）も自由セルも通りません。津田沼でも占有は 9.4 万セル
（`int8` の 51〜100）なので 1.6 億回程度で、起動が目に見えて延びる量ではありません。
...
0.5 だったので、前進の 3 つはここで断片と同値になりました。**後退も 2026-08-09 に
5/3 倍の 0.3333 から断片の 0.2 へ戻したので、いまは 6 つとも断片と同じです**（根拠は
未記録。両 override が同じ配列を明示しているのは、地図ごとの差を縦に見るためです）。
...
`overrides/map_19f.yaml` の `cost_drawing_threshold` は 180 → 108 →
**100**（2026-08-09）です。表示専用で、2026-07-29 の実測（分布）は 1 手 0.3 m の頃の
ものなので、同じ距離（≒54 m）を指すよう 0.6 倍したのが 108 でした。100 はそこからの
丸めで、**根拠は記録されていません**（≒50 m 相当）。**分布は測り直していません。**

**未検証** — 実機では走らせていません。旋回付きが実際に選ばれるようになったか、
狭隘部がどう変わるか、`velocity_smoother` の既定 `max_accel` 2.5 m/s²（`joy_teleop`
が置いている 0.9 の 2.8 倍）で 0 → 0.5 の立ち上がりに制御基板のループが追いつくかは
どれも測っていません。飽和はエラーを出さず**指令より遅く・左右バラバラに回る**だけ
...
値**で、同梱の 2 地図はどちらも `overrides/` で上げてあります（19F が 100、津田沼が
...
**いま入っているのは 100 です。** 計測当時は 1 ステップ 0.3 m でしたが、断片の
`action_forward_m` が 0.5 m へ上がったぶん、同じ距離が 0.6 倍の手数で済みます
（下の `action_forward_m` の節）。180 × 0.6 = 108 が ≒54 m を指す値で、**2026-08-09 に
そこから 100（≒50 m）へ丸めました**（根拠は未記録）。分布そのものは測り直していません。
...
`window_cost_drawing_threshold` は 2026-08-09 の上流の整理で消えたので、地図側で
上限を上げるほど窓のほうは階調が潰れます（表示だけの話です）。
...
* **全域スライスの 1 枚が重い地図です。** `/value_function` は 94 万セル（19F の
  ...
  走行中に `/cmd_vel_mux` が 2 秒おきに途切れるなら、まず `value_publish_interval_ms`
  を疑ってください（0 にすると掃き中の出し直しが止まって solve 完了時の 1 枚だけに、
  負にすると配信そのものが立ちません。配信の on/off だった `publish_value_function`
  は 2026-08-09 の上流の整理でこのキーに吸収されました）。
...
**2 にしている理由は solve を軽くするためです。** 状態数が 1/4 になるぶん solve も
伝播も速くなります。**2026-08-09 にこの地図の `solver` を密へ戻した（`active_reloc`
がアウトオブコアを受け付けないため）ので、いまは必須です** —— 密の scale 1 は
2.53 GB で 4 GB 機に載りません。compact だった頃は必須ではありませんでした
（2026-07-29 の実機は compact scale 1 で solve 29.25 s / RSS 833 MB で通っています）。

**密はこの scale 2 とセットです。** 密の常駐は 80 B/state — `states` 56 B に
...
メモリ上限を見るのはノード側です。地図の実寸はノードしか知らないので、そちらのほうが
正確に判定できます。**上限のキー（`dense_limit_mb`）は 2026-08-09 の上流の整理で消え、
`/proc/meminfo` の `MemAvailable` そのものが基準になりました** —— 超えたら起動を止め、
半分を超えたら警告です。書いても読まれません。

**この地図は `waypoint_prefetch` を `true` にしてあるので、表の数字は 2 倍で
読んでください。** 先読み中は価値関数が 2 本生きます。**2026-08-09 に密へ戻したので
いまは scale 2 で 1.3 GB が匿名メモリに乗ります**（密には `compact_ram_limit_mb` の
ようなディスクへ逃がす口がありません）。**4 GB 機では先読みのほうを外してください。**
compact だった頃は sink が 0.095 GB × 2 = 0.19 GB で、断片の
`compact_ram_limit_mb`（4096 MB）に収まるのでディスクへも出ませんでした。
scale 1 なら 5.0 GB です。ノードのメモリ判定（`MemAvailable` との比較）が見ているのが 1 本ぶんか
...
* **密の 1 掃きの実時間**: 掃き速度の host 実測は 5.23 M cells/s なので、19F の scale 2
  ...
  です。ノード既定の `global_sweep_duty: 25`（1 コアの 25%）だとその 4 倍かかります。
  `nav2/vi_planner.yaml` も**この既定のまま**です（2026-08-09 の上流の整理までは
  `global_sweep_budget_ms: 20` / `global_sweep_idle_ms: 60` という 2 つのキーで、
  同じ 25% を掃き時間と待ち時間で書いていました）。
...
* **ロックの持ち方**が肝です。10 Hz の追従ループは同じ `Mutex<PlannerCore>` を
  ...
  `global_sweep_duty` の割合だけロックを握って掃き、残りは手放して待ちます。
  上げるほど伝播は速く、走行がぎくしゃくします（100 で掃きっぱなし）。
...
* **見るトピックを間違えない。** `/value_function`（全域スライス）は以前は
  **solve が実際に走ったときしか出ていません**でした。BT の 1 Hz の再計画は同じ
  ...
  （`value_publish_interval_ms` が正、かつ `global_sweep: true` のとき）。
...
**この上限が効くのは compact で解いている地図だけです。** `map_19f` は 2026-08-09 に
密へ戻したので、いまこの値は**読まれません**（compact の頃も scale 2 の sink 0.095 GB
なので 512 MB でも 4096 MB でも RAM に載り、値を上げても何も変わりませんでした）。
残る `map_tsudanuma` は `compact_sink_dir` が空 = RAM 出力なので、この上限が
そのまま効きます。**`map_tsudanuma` だけが
...
密ソルバ側の上限はキーではなく `MemAvailable` そのものです（2026-08-09 の上流の整理まで
は `dense_limit_mb`、既定 1500 MB・断片では 4096 MB でした）。こちらは「超えたら退避する」
ではなく **「超えたら起動を止める」** で、黙って確保して OOM killer に落とされるより理由を
出して止めるためのものです。**マシンが持っている値で決まるので、4 GB 機と 8 GB 機で
別々に書く必要が無くなりました。**

**2 つとも 2048 → 4096 へ上げたのは 2026-08-04 で、Pi 5（8 GB）が前提です**
（うち密側は上のとおりキーごと無くなり、いま断片に残っているのは
`compact_ram_limit_mb` の 4096 だけです）。
...
`compact_ram_limit_mb` を下げる（sink を早めに退避させる）、コンテナ側で `mem_limit` を
掛ける、のいずれかです。

このキーは元々、当時の広域専用ノード `vi_global_planner` にしかありませんでした。
2026-08-04 に `vi_planner` にも同じ既定値（512 MB）・同じ判断順で実装し、2026-08-08 に
あちらのノードごと消えたので、いまの宛先は `vi_planner` だけです。**2026-08-09 に
ノード既定が 512 MB から 0 = `MemAvailable` の半分へ変わりました**（明示できるキーとして
残っているのは、sink を RAM に置いてよいかが同居プロセスと保存先の速さ次第だからです）。
自動退避先は `/tmp/vi_planner_sink`。
