...
メモリ上限を見るのはノード側の `dense_limit_mb`（既定 1500 MB、断片では 2048 MB）
...
* **密と compact で掃き方が違います（結果は同じ）。** 密は上のとおり `states` を
  ...
  * **常駐メモリはタイル 1 枚ぶんだけ**（0.25 m/cell で約 1.9 MB、追従パッチ
    3.5 MB と合わせて 6 MB 弱）。
  * **伝播の実時間は未計測**です。密と同じく起動ログの
    `global sweep done in ...` に出ます。
...
19F は両ノードとも compact ですが、scale 2 の sink 0.095 GB は 512 MB でも
...
明示するほうが安全です。19F は sink が RAM に載る大きさなので、いまのところこの
経路には入りません。
