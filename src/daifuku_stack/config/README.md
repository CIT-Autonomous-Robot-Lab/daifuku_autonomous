...
### `vi_planner` / `vi_global_planner` の `action_forward_m`（旋回付きの前進量）

`action_forward_m` の 4 番目と 6 番目（`rightfw` / `leftfw`）を、ノード既定の 0.2 から
直進と同じ **0.3 m** へ上げてあります。津田沼の override も同じ比（5/3 倍）で
0.3333 → 0.5 にそろえています。
...
依存しません**。したがって 1 m 進むのに要する手数は直進が 1/0.3 = 3.33 手、
旋回付きが 1/0.2 = 5.0 手で、**旋回付きだけが 1.5 倍高くつきます**。
実機では曲がりながら進む手がほとんど選ばれず、その場旋回 → 直進の繰り返しに
なっていました（2026-08-05）。0.3 へそろえると、旋回付きは「直進と同じだけ進んで
20 度向きも変わる手」になり、向きを変える必要があるところでは直進と同じ距離効率で
選べるようになります。

代償が 2 つあります。

* **旋回半径が 0.573 m から 0.859 m へ広がります**（`linear.x / angular.z` =
  0.3 / 20 deg/s）。曲がりきれない狭さでは価値反復が `right` / `left`（前進 0）を
  選ぶだけなので**壊れはしません**が、狭いところで曲がりながら進めるようには
  なりません。
* **薄い壁のすり抜けが旋回付きにも広がります。** 遷移は着地セルしか見ないので、
  1 手 0.2 m のときは 0.2 m より薄い壁しか跳び越えられませんでした。0.3 m では
  直進と同じ 0.3 m まで広がります。ただし**直進が最初から 0.3 m** なので、地図に
  要求される最小の壁の厚みは変わりません（新しい失敗の種類は増えない）。

`max_fw`（= 配列の絶対値の最大）は 0.3 のまま変わらないので、compact 追従の
パッチ寸法（`new_patch` の `reach_bound`）にも影響しません。**未検証** — 実機で
旋回付きが実際に選ばれるようになったか、狭隘部の挙動がどう変わるかは測って
いません。
...
値**で、同梱の 2 地図はどちらも `overrides/` で上げてあります（19F が 180、津田沼が
...
p99 300 でした。**`overrides/map_19f.yaml` の 180 はここから採った値**です。p90 と
p99 の間なので、運用上通る範囲に階調を集中させ、遠い裾だけ飽和させる選び方になります。

別の地図では分布が変わります（飽和が広ければ小さすぎ、階調が下位に偏っていれば
大きすぎ）。**`overrides/map_tsudanuma.yaml` の 600 はこの実測にあたるものがなく、
地図の広がりから置いた見込みです**（未計測）。あちらは 1 ステップが `action_forward_m`
0.5 m（断片の 5/3 倍）なので 600 ≒ 300 m にあたり、294.4 x 200 m の地図をほぼ端まで
...
**代償**: 逃がさない代わりに、その 4 GiB は匿名メモリとして居座ります。Pi4（4 GB）で
