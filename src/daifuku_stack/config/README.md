...
値**ですが、いまは 19F も断片（`nav2/vi_planner.yaml`）の 60 のまま使っています
（`overrides/map_19f.yaml` では上書きしていません）。
...
p99 300 でした。遠くまで階調が要るときは 180 にしてください。p90 と p99 の間なので、
運用上通る範囲に階調を集中させ、遠い裾だけ飽和させる選び方になります。

別の地図では分布が変わります（飽和が広ければ小さすぎ、階調が下位に偏っていれば
大きすぎ）。いまは同梱の 2 つともこれを上書きしていないので、どちらも 60 で動きます。
...
* **密ソルバ専用です。** compact 側にも共有場自体はあります（狭域が動かした窓を
  毎 tick sink へ書き戻し、それを裏付ける `local_penalty` を 1 B/セルの全域表で
  覚えるので、広域のロールアウトからも次に起こし直したパッチからも見えます）。
  無いのは**掃く手立て**のほうで、この機能は全域ぶんの `states` と `sweep_orders`
  （80 B/state）を要求します。compact はまさにそれを持てないから compact なので、
  同じコードは使えません。したがって compact では狭域の効果は窓の周りに留まり、
  20 m 先から降りてくるロールアウトは塞がった通路を選び続けます。
