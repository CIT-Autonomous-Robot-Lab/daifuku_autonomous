...
`19f.yaml` の内蔵推定器 4 つで、あれは密ソルバが要るので津田沼の 2 つには置けません）。
...
* `sensor_reset`: README の表は既定 `true` ですが、`emcl2_node.cpp` の
  ...
  同梱の 3 地図はどれも `overrides/` が `false` にしていたので変わりませんが、
...
`overrides/*.yaml` の `emcl2` の節へ（そのときは 3 つの override にキーをそろえて並べる）。
...
旋回付きが 0.2 → 0.5 で、**後退（2 番目）は 0.2 のまま**。3 つの override も 2026-08-09 から
...
3 つの override の `velocity_smoother` の節です。実際の値は
...
**`/local_window_value`（追従の窓）も同じ上限を使います。** 窓だけ別の値にできる
...
* `local_planner:=vi`（両アクション）と `local_planner:=nav2`（同じ `vi_planner` を
  ...
  密な状態配列が要るので、全域ではなくロボット近傍のパッチ（追従の窓 + 遷移到達距離 +
...
### `overrides/tsudanuma_mugimaru.yaml` の `map_scale: 2`

**同じ `2` でも 19F とは意味が違います。** `map_scale` は倍率なので、効き方は
**地図そのものの解像度**で決まります。この地図は元から 0.1 m/cell なので、`2` は
0.20 m/cell — 19F の `2`（0.05 → 0.10 m/cell）ではなく、津田沼の `5`（0.05 →
0.25 m/cell）に近い側です。

| | 19f | tsudanuma | tsudanuma_mugimaru |
| --- | --- | --- | --- |
| 地図 | 915x577 @0.05m | 5888x4000 @0.05m | 2500x1581 @0.1m |
| `map_scale` | 2 | 5 | 2 |
...
——2 倍プーリングなら通路が片側最大 0.1 m しか細らず、楽観が要る `0.25 m/cell` には
...
**そこで 2026-09-02 に、順路から 5m の回廊だけを残した navigation の地図を作りました**
...
自由空間が 36,801 → 12,528 m² になり、**フル solve は 27.95 → 13.07 秒**
...
32.44 秒 = 走行 89.85 秒の 1/2.8 に収まります。**実機の solve が本ホストの 2.8 倍まで
...
実測 654.8 MB は `states` 444.7 MB + `sweep_orders` 210.1 MB です。**2026-08-20 の上流の更新で
全域伝播が能動集合になり、このノードは掃き順をもう読みません**（それでも 210 MB 乗るのは
`set_map_with_occupancy_grid` が無条件に作るため。落とすには vi_lib 側に口が要ります）。compact だった頃は必須では
...
### `vi_planner` の追従ウィンドウ（`local_shape: capsule`、前後 2.5 m・横 2.0 m）

追従（`follow_path`）がスキャンで補正し、毎 tick 価値反復を回し直す範囲です。**本家は
±1 m の地図軸正方形固定**で、2026-08-09 に半径だけ、2026-08-20 に形が動かせるようになりました。
断片では**カプセル**（幅 `local_xy_range_lat` の円を進行方向の線分に沿って掃いた形）にして
います。

| キー | 値 | 意味 |
| --- | --- | --- |
| `local_shape` | `capsule` | ロボット座標系に載る。前縁でも横幅 `lat` を保つ |
| `local_xy_range` | 2.5 m | 前後の基準半径 r（停止中の前後到達距離そのもの） |
| `local_xy_range_lat` | 2.0 m | 横半径 b。前縁まで一定 |
| `local_shape_ratio_max` | 1.25 | 進行方向への伸縮の倍率上限 |

**この 4 つで「停止中は全周 2.0 m 以上、全速前進で前 3.0 m」になります。** 前後の半軸は
`a_fwd + a_bwd = 2r` を保ったまま走行方向へ移り（面積 = 掃きコストが速度で動かないため）、
`[r/ratio_max, min(2r - r/ratio_max, r·ratio_max)]` = `[2.0, 3.0] m` に収まります。
**到達距離そのものは `max(半軸, lat)` です** — カプセルは半軸から `lat` を引いた線分に円を
掃いた形で、その線分長が負になると 0 でクランプされるので、**前後の到達距離は `lat` を
下回りません**（`ratio_max` を上げても同じ。ノード既定の 1.5 なら半軸は 1.667 m まで縮み
ますが、後方の到達は `lat` = 2.0 m のまま）。つまりどの向きでも最低 2.0 m です。伸縮の速さは
`local_shape_slew_per_sec`（既定 1.0 m/s = 10 Hz で 1 tick 0.1 m）なので、2.5 m から 3.0 m
まで 0.5 秒かかります。**`ellipse` ではなく `capsule` なのは、楕円が前縁で点に尖って
「障害物が最初に入ってくる所」の横被覆を落とすためです**（上流の推奨）。

**代償が 2 つあります。どちらもエラーも警告も出ません。**

* **1 tick の refine が窓 1 周を終えられなくなります。** `refine_pass_until` は毎 tick
  窓の x 最小端から掃き直し、**途中再開のカーソルを持ちません**。`refine_budget_ms`（40 ms）
  で足りないぶんは、窓の x が大きい側が一度も掃かれないまま次の tick になります。
  **x は地図座標の軸**なので、機体が +x へ走っているときは掃かれないのが**前縁**、
  -x へ走っているときは後縁です（悪いのは前者）。
  窓の面積は `πb² + 2b(2r - 2b)` = 16.57 m²（前後の配分によらず一定）なので、
  地図ごとに次のようになります（Pi 4 の掃き速度 約 1 M states/s から）。
  ...
  **19F と `tsudanuma` は `greedy`** で、`optimal_action` が掃いた結果そのものなので
  **判断が丸ごと掃き待ち**になります。そのときは `refine_budget_ms` を上げるか窓を
  縮めてください。**`tsudanuma_mugimaru` だけ `dwa`** で、あちらの軌道棄却は `penalty` を
  直に読むので（スキャン注入は毎 tick 効く）**障害物回避そのものは生きます**。掃き待ちに
  なるのは生き残った候補の順位付け（V̂ = 掃いた値）だけです。ただし `dwa` が候補全滅か
  無進展ガード（どちらも V̂ を見る）で手を引くと `greedy` へ落ちるので、**その落ち先は
  掃き待ちの `optimal_action` です**。**未検証**（上の時間は式に値を入れた
  見積もりで、実機では測っていません）。
* **`tsudanuma` では形が変形しません。** 伸縮の不感帯は 0.3 セル
  ...
  **一度も超えません**。あの地図の窓は全周 2.5 m の静止カプセルのままです（最低 2 m の
  要求は満たします）。19F は 0.03 m なので、全速の 6 割から伸び始めます。

**compact の追従パッチも大きくなります。** パッチは「変形の上限込み」の外接半径
`max(r·ratio_max, b)` = 3.125 m で決まるので、津田沼では 27×27×60 = 2.5 MB から
59×59×60 = 約 11 MB になります（8 GB 機なので問題にはなりません）。実寸は起動ログに
出ます（下の `global_sweep` の節）。

**スキャン由来の `local_penalty` を撒く範囲も 16.6 倍になります。** ウィンドウの外の
penalty は誰も消さない（下の `global_sweep` の節）ので、19F のように有効ビームの 28% が
壁を貫通する地図では、誤検知の残り方も同じ倍率で増えます。**未検証。**

### `vi_planner` の `global_sweep`（狭域 → 広域のフィードバック）

`vi_planner` は 1 本の価値関数を広域（`compute_path_to_pose` のロールアウト）と狭域
（`follow_path` の窓。上節のカプセル）で共有します。狭域はスキャンのヒット点に `local_penalty`
...
* **密と compact で粒度が違うだけです（考え方も結果も同じ）。** どちらも「値が動いたところ」
  から遷移の届く距離だけ膨らませた**能動集合**を掃きます。密はセル単位（動いたセルのビット
  ボードを遷移到達距離ぶん dilate して次ラウンドの候補にする）、compact に全域の `states` は
  無いので sink を**タイル単位**で修復します（1 タイル = 更新する interior 16 セル角 + 遷移が
  届く halo。halo を凍結境界にして interior を掃き、変わった列を sink へ返し、変化から遷移
  到達距離以内のタイルを待ち行列へ入れ直す）。どちらも全走査と同じ不動点に達します
  （`vi_lib` の `conformance_resweep_propagates_a_raised_penalty` が、収束後の場に penalty を
  足してから掃き直した結果を全ソルバぶん `Reference` と突き合わせています）。
  **2026-08-20 の上流の更新までは密だけが地図を丸ごと Gauss-Seidel で掃いていました。**
  * **仕事量は地図の大きさではなく、変化が及ぶ範囲に比例します（密も compact も）。** 値が
    動かなかったセル／タイルは隣を起こさないので波面はそこで止まります。逆に全域へ波及する
    変化なら全域 1 掃きぶん働きます（津田沼 0.25 m/cell で約 3,700 タイル訪問 = 読み 1.3 GB /
    書き 0.7 GB）。**打ち切りではありません。**
  * **常駐メモリはタイル 1 枚ぶんだけ**（0.25 m/cell で約 1.9 MB、追従パッチ 3.5 MB と
    合わせて 6 MB 弱）。**追従パッチの実寸は 2026-08-20 から起動ログに出ます**
    （`vi_planner: follow patch <辺>x<辺>x<θ> = <MB> (window ... + transition reach ...)`）。
    パッチは窓の寸法の 2 乗で効くので、`local_xy_range` を広げたり `local_shape` を
    `square` 以外にしたりすると数百 MB に届きます（`local_shape_ratio_max` が外接半径に
    入るのは `square` 以外のときだけです）。
    **止める門は無いので、気付く手掛かりはこのログだけです。**
  * **実機での伝播の実時間は未計測**です。host の合成テスト（5 m 四方・0.05 m/cell）では、
    通路を幅いっぱいに塞いでから 2.5 m 先の値が落ち着くまで追従 119 tick（= 12 秒相当）・
    タイル訪問 358 回でした。
* **反応速度**: 合成テスト（密。**全走査だった頃の実測**）は **1 掃き目で広域の価値が動き**、
  30 掃きでほぼ収束、完全収束（Δ=0）は約 80 掃き。収束を待つ必要はありません（掃くたび不動点へ単調に近づき、
  経路が変わるのは遥かに手前）。compact のタイル修復も同じで、1 タイルあたり平均 36 訪問
  （= 72 パス）で Δ=0 に達しました。
* **密の実時間**: 地図を丸ごと掃いていた頃の見積もりは、host 実測 5.23 M cells/s から
  19F の scale 2（794 万状態）で 1.5 秒、Pi4 は 5〜8 倍遅いので **8〜11 秒**、
  `global_sweep_duty: 25`（1 コアの 25%）だとその 4 倍でした。**能動集合になった
  いまはこれが全域へ波及したときの上限で、普段の 1 回はもっと短くなります**（どれだけ
  短いかは変化の広がりしだいなので見積もれません）。予算の刻み（ノード内の定数）の単位も
  セルから**状態**へ変わりました（候補セル 1 つにつき θ 60 層ぶん回る）。2026-08-04 に
  `global_sweep_duty` を 60 / 100（37% 相当）まで上げましたが、同日の実機で機体が 1〜2 秒
  おきに固まったので戻しました（犯人の切り分けは未了。2026-08-09 の上流の整理までは
  `global_sweep_budget_ms` / `global_sweep_idle_ms` の 2 つのキーで同じ 25% を書いていました）。
  **実測値は起動ログの `global propagation settled in ...` に出ます。未検証。**
...
* **止まるとき**: 能動集合が空になったら（密は次ラウンドの種が空、compact は待ち行列が空）、次に
  狭域が場を動かすまで止まります（CPU を焼き続けません）。ただし**走行中はまず止まりません** —
  壁が窓に入っていれば `set_local_cost` が毎 tick penalty を塗り直すので、次の伝播が
...
* **見るトピックを間違えない。** `/value_function`（全域スライス）は以前は**solve が実際に
  ...
  `/local_window_value`（追従の窓）だけで、これは機体と一緒に動くため、**離れると固まったままの
  全域が下から出てきて「上書きされた」ように見えます**。いまは掃きスレッドが 2 秒ごとに
  出し直します（`value_publish_interval_ms` が正、かつ `global_sweep: true` のとき）。
  **`value_publish_interval_ms: 0`（= 落ち着いたときだけ配信）にすると、掃きスレッドが出すのは
  伝播が不動点に達したときだけになります。** 壁沿いを走っているあいだは毎 tick 次の伝播が
  積まれて不動点に達しないので、**エラーも警告も出ないまま全域スライスが固まったまま**に
  なります（走行中も動かしたいなら既定 500 ms のまま）。
...
ログは 2 種類出ます。走行中は前者がまず出ないので、後者で見てください。**単位は密が `cells`、
compact が `tiles`** で、行そのものはどちらの経路でも同じです（2026-08-20 の上流の更新まで
進捗のほうは compact でしか出ませんでした）。

```
vi_planner: global propagation settled in 3.4s (358 tiles)             # 待ち行列が空になった
vi_planner: global propagation running for 6.0s (412 tiles done, 27 queued) # 2 秒ごとの進捗
```
