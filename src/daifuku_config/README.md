...
`goal_margin_radius` は `0.75` にしてあります。ゴール圏のセル数は「半径 ÷ 内部解像度」
なので、0.20 m/cell では断片の `0.3` だと 1.5 セルに潰れます。**そろえてあるのは
メートルのほうでセル数ではありません** — 同じ場所の `tsudanuma` と同じ許容差にしてあり
（2026-08-26 に 8 手・前進 0.8 m へ上げたときに `0.6` から）、セル数は 3.75（あちらは
0.25 m/cell で 3、19F は 0.10 m/cell の `0.3` で 3）です。
