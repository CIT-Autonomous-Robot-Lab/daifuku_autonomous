...
**本体ドライバは受けた速度をそのままステップ周波数にする**（自前実装も公式実装も同じ）ので、
段差のある指令はそのまま段差のある周波数になり、ステッパは自起動域を超えたところで**脱調
します** — 指令より遅く、左右バラバラに回るのに**エラーは何も出ません**。ブーストの0.8 m/s
はステップ726 Hz相当で、静止からいきなり出せる値ではありません。
...
すぐ止まりそうに見えますが、脱調して惰性で滑るので実際は`linear_decel`で降ろすほうが速く
止まります。**即断したいときはモータ電源**（BACKを2秒長押しして離す）のほうです。
