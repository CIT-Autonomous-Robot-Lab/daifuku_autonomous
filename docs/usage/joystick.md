...
| RBを押しながら | ブースト（上限が**0.8 m/s**・1.5 rad/s に上がる） | |
...
| 値 | 既定 | 意味 |
| --- | --- | --- |
| `linear_accel` | 0.9 m/s² | 0 → 0.8 m/s に0.89秒 |
| `linear_decel` | 1.8 m/s² | 0.8 m/s → 0 に0.44秒（0.18 m） |
| `angular_accel` | 6.0 rad/s² | 0 → 1.5 rad/s に0.25秒 |
| `angular_decel` | 12.0 rad/s² | 1.5 rad/s → 0 に0.13秒 |

**旋回だけ並進より強くしてあります。** 旋回1.5 rad/sはトレッド0.35 mでは車輪の差が
±0.26 m/s＝ステップ238 Hz相当で、ブーストの726 Hzよりずっと軽いためです。ただし
`publish_rate`（20 Hz）の刻みで出す以上、`angular_decel` 12 rad/s²は3刻みで降りきる速さで、
**これ以上上げても刻みが粗くなるだけ**（実質は段差）になります。

**減速を加速の2倍にしてあります。** 立ち上がりが緩いのは操作感の問題で済みますが、止まるのが
遅いのは安全側ではないためです。不感帯を出た瞬間の下限値への飛び（0 → 0.1 m/s）も、ブースト
を離したときの落ち（0.8 → 0.35 m/s）も、前後の切り替えも同じ刻みを通ります。前後を入れ替える
ときは、0までを減速側・0から先を加速側の上限で刻むので、**0を跨いだ瞬間に緩みません**。

**`/joy`が途切れたときの停止距離もこれで決まります**（0.8 m/sから0.18 m）。段差で0に落とせば
