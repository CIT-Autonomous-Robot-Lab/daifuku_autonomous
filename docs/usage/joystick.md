...
| 値 | 既定 | 意味 |
| --- | --- | --- |
| `linear_accel` | 0.6 m/s² | 0 → 0.8 m/s に1.33秒 |
| `linear_decel` | 1.2 m/s² | 0.8 m/s → 0 に0.67秒（0.27 m） |
| `angular_accel` | 2.0 rad/s² | 0 → 1.5 rad/s に0.75秒 |
| `angular_decel` | 4.0 rad/s² | 1.5 rad/s → 0 に0.38秒 |
...
**`/joy`が途切れたときの停止距離もこれで決まります**（0.8 m/sから0.27 m）。段差で0に落とせば
