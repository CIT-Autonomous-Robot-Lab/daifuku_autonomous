...
- **エンコーダと指令パルスを別のパラメータで数えます。** この機体は車輪 1 回転が
  ...
  [`src/daifuku_config/README.md`](../../src/daifuku_config/README.md)）。`odom` は
...
- パルスカウンタが生きているとき、`odom` の Twist は**指令値ではなく実測値**です。
  `src/daifuku_config/bringup/sensors/mid360_ekf.yaml` はこのメッセージから vx と vyaw だけを取るので、
...
`src/daifuku_config/README.md` に書いてあるとおり、rtmouse は I2C が 1 回タイムアウトすると
...
パラメータに出してあるので、`src/daifuku_config/bringup/robot/raspicat_driver.yaml` を直せばコードは
触らずに済みます。
...
[`src/daifuku_config/README.md`](../../src/daifuku_config/README.md)。
...
`pulses_per_revolution` は 1 つの値で全速度には合いません（不足が距離ではなく経過時間に
比例する。**原因は未解明で、カウンタの読み出しではない**）。詳細は
...
| 荷重下の実速度 | 0.1 m/s を 10 秒指令して巻尺と `/odom` を比べる。較正済みなので一致するはずで、ずれるならすべりか較正 | `steps_per_revolution` |
| **電源** | 制御基板は 40 ピンヘッダ経由で Pi に 5V を供給する。Pi 5 の電流で足りるか。無負荷では `vcgencmd get_throttled` が `0x0` | — |
| 基板の閉ループの余裕 | 速度の閉ループは制御基板の中にある。**無荷重は 2026-08-08 に済み**（0.10〜0.40 m/s で実速度の落ちなし）。残っているのは**荷重をかけた側**で、荷重をかけても巻尺が指令に追いつくか。落ちるなら基板のループが飽和している | — |
