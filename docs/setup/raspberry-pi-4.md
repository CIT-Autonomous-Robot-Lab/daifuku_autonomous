...
`config/bringup/robot/raspicat.yaml` の `use_pulse_counters` は `false`（`cmd_vel` の積分）
...
- Nav2 を回転中に止めるとゼロ速度が届かず、`odom` が回り続けます。その幻の回転を
  emcl2 が打ち消そうとして `map->odom` まで振り回されます。**まずゴールを取り消して
  ください**（操作パネルの「中断」、パッドの START 長押し、送った端末の Ctrl-C）。
  仲裁（`twist_mux:=true` が既定）は自律側の `/cmd_vel` のほうが優先度が高いので、
  ゴールが生きているあいだ `control.sh stop`（`/cmd_vel_teleop` へゼロ）は
  **エラーも出さずに無視されます**。自律側が 0.5 秒黙ってからならそちらで止まります。
  `twist_mux:=false` で立てているなら宛先は `/cmd_vel` です。
...
- **`use_composition` の既定 `False` は意図的です。** `True` にすると参加者あたりの
  ...
  心拍も途絶えます。`config/stack/lifecycle_bond.yaml` の `bond_timeout: 60.0` も同じ事情
...
**測距センサだけは切ってあります**（`config/bringup/robot/raspicat.yaml` の
...
| 指令周波数と実速度 | 0.1 m/s を指令して巻尺で実測。実測較正は Pi 5 で済ませてあり（**床の上で** エンコーダ 1073 / 指令パルス 447。2026-08-08。制御基板側の性質なので機種によらない）、ずれるならすべりか較正。**車輪を浮かせて測らないこと** — 浮かせて得た 1118 / 570 は床では 27% ずれていた | `steps_per_revolution` |
...
| 荷重下の実速度 | 上の実速度を無荷重と荷重で比べる。速度の閉ループは制御基板の中にあるので、荷重側でも巻尺が指令に追いつくはず。落ちるなら基板のループが飽和している | — |
...
| 項目 | 分かっていること | 出どころ |
| --- | --- | --- |
| 寸法 | 車輪径 200 mm / トレッド 350 mm（2026-08-03 の実測値）。上流 raspicat の既定 152.4 / 279.18 mm に戻すと並進が 1.31 倍ずれる | [`config/README.md`](../../config/README.md) |
